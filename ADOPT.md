...
3. **Graphify.** If the project does not use Graphify, delete `docs/agents/graphify.md`, its row in the task guides table, the Graphify bullet under "While you work" in `AGENTS.md`, the `graphify update` item in "Definition of done" (renumbering the items after it), and the word from the `tooling` tag description in `docs/decisions/README.md`. `rg -i graphify` must then find nothing.
4. **End-to-end checks.** Fill the placeholders in `docs/agents/end-to-end.md` with the project's own commands: Playwright for a browser surface, Xcode and Swift for an Apple one. Delete the section that does not apply (a project with no browser deletes "In a browser"; one that ships nothing for Apple platforms deletes "On Apple platforms"), and trim "Which tool" to the tools that remain. If the project has no user-facing surface at all, delete the guide and its row in the task guides table, and delete the matching rule in `AGENTS.md`. If the surface is planned but does not exist yet, keep the guide and say in each section that the command arrives with the first surface, rather than inventing one.
